...
contract MemeCollection is ERC721, ERC721Enumerable, ERC721URIStorage, AccessControl, ReentrancyGuard, IERC2981 {
    ...
    uint256 public constant MAX_MINT_PER_USER = 9;
    ...
    function setMinterContract(address _minterContract) external onlyRole(ADMIN_ROLE) {
        require(_minterContract != address(0), "MemeCollection: Invalid minter contract address");
    ...
    function mintForUser(address to, string memory metadataURI) external onlyRole(MINTER_ROLE) nonReentrant {
        require(to != address(0), "MemeCollection: Cannot mint to zero address");
        require(_tokenIdCounter.current() <= MAX_SUPPLY, "MemeCollection: Max supply reached");
        require(userMintCount[to] < MAX_MINT_PER_USER, "MemeCollection: User mint limit reached");
