    ...
    testConfigs.forEach(config => {
        describe(`${config.name}`, function () {
            ...
            describe("Token URI Management", function () {
                beforeEach(async function () {
                    // New backend flow: mint with empty URI first, then set URI separately
                    await nftCollection.setMinterContract(minter.address);
                    await nftCollection.connect(minter).mintForUser(user1.address, "");
                });

                it("MINTER_ROLE can setTokenURI on a minted token", async function () {
                    const ipfsURI = "ipfs://QmCorrectTokenId1";
                    await nftCollection.connect(minter).setTokenURI(1, ipfsURI);
                    expect(await nftCollection.tokenURI(1)).to.equal(ipfsURI);
                });

                it("ADMIN_ROLE can setTokenURI directly (manual backstop)", async function () {
                    await nftCollection.connect(owner).setTokenURI(1, "ipfs://QmAdminFixed1");
                    expect(await nftCollection.tokenURI(1)).to.equal("ipfs://QmAdminFixed1");
                });

                it("setTokenURI can be called again to update URI (no lock)", async function () {
                    await nftCollection.connect(minter).setTokenURI(1, "ipfs://QmFirst");
                    await nftCollection.connect(minter).setTokenURI(1, "ipfs://QmUpdated");
                    expect(await nftCollection.tokenURI(1)).to.equal("ipfs://QmUpdated");
                });

                it("setTokenURI emits TokenURISet event", async function () {
                    const ipfsURI = "ipfs://QmEventTest";
                    await expect(nftCollection.connect(minter).setTokenURI(1, ipfsURI))
                        .to.emit(nftCollection, "TokenURISet")
                        .withArgs(1, ipfsURI, minter.address);
                });

                it("setTokenURI reverts for non-existent tokenId", async function () {
                    await expect(nftCollection.connect(minter).setTokenURI(999, "ipfs://Qm"))
                        .to.be.revertedWith("NFTCollection: Token does not exist");
                });

                it("setTokenURI reverts with empty URI", async function () {
                    await expect(nftCollection.connect(minter).setTokenURI(1, ""))
                        .to.be.revertedWith("NFTCollection: URI cannot be empty");
                });

                it("Random wallet cannot call setTokenURI", async function () {
                    await expect(nftCollection.connect(user2).setTokenURI(1, "ipfs://Qm"))
                        .to.be.revertedWith("NFTCollection: caller is not minter or admin");
                });
            });

            describe("Minter Contract Role Revocation", function () {
                // NOTE: setMinterContract does NOT auto-revoke old minter. Admin must
                // call revokeRole manually to remove a stale minter after an upgrade.

                it("Old minter STILL has MINTER_ROLE after replacement (no auto-revoke)", async function () {
                    const MINTER_ROLE = await nftCollection.MINTER_ROLE();

                    await nftCollection.setMinterContract(minter.address);
                    await nftCollection.setMinterContract(user2.address); // replace

                    // Both addresses hold the role until admin revokes manually
                    expect(await nftCollection.hasRole(MINTER_ROLE, minter.address)).to.be.true;
                    expect(await nftCollection.hasRole(MINTER_ROLE, user2.address)).to.be.true;
                });

                it("Admin can manually revoke old minter role after upgrade", async function () {
                    const MINTER_ROLE = await nftCollection.MINTER_ROLE();

                    await nftCollection.setMinterContract(minter.address);
                    await nftCollection.setMinterContract(user2.address); // replace

                    // Admin revokes old minter manually
                    await nftCollection.connect(owner).revokeRole(MINTER_ROLE, minter.address);

                    expect(await nftCollection.hasRole(MINTER_ROLE, minter.address)).to.be.false;
                    expect(await nftCollection.hasRole(MINTER_ROLE, user2.address)).to.be.true;
                });
            });
        });
